    ...
    daysTag.querySelectorAll("li").forEach((liElement, index) => {
        ...
        liElement.addEventListener("click", () => {
            ...
            if (liElement.classList.contains("selected")) {
                ...
                scheduleResult.innerText = `Você agendou pro dia ${id} de ${months[currMonth]} de ${currYear}`;
                scheduleResult.classList.add("none");
            }
        });
    });
}
...
const callSchedule = (hours, date) => {
    ...
    if (selectedHour) {
        scheduleResult.innerHTML += ` às ${selectedHour} horas`;
        scheduleResult.classList.remove("none");
    } else {
